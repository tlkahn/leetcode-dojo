Implement the =BSTIterator= class that represents an iterator over the *[[https://en.wikipedia.org/wiki/Tree_traversal#In-order_(LNR)][in-order traversal]]* of a binary search tree (BST):
